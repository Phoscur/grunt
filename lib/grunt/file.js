...
// Get the paths of all global and local "grunt-" named npm modules.
exports.gruntLibs = function(callback) {
  function parseLists(lsGlobal, lsLocal) {
    var libPaths = {};
    function findKeys(obj) {
      Object.keys(obj).forEach(function(name) {
        if (!libPaths[name]) {
          libPaths[name] = obj[name].path;
        }
        findKeys(obj[name].dependencies);
      });
    }
    findKeys(lsLocal.dependencies);
    findKeys(lsGlobal.dependencies);
    Object.keys(libPaths).forEach(function(name) {
      if (!/^grunt-/.test(name)) {
        delete libPaths[name];
      }
    });
    callback(false, libPaths);
  }

  npm.load({loglevel: 'silent'}, function(err, npm) {
    var lsGlobal, lsLocal;
    npm.config.set('global', false);
    npm.commands.ls({global: false}, true, function(err, data) {
      lsLocal = data;
      npm.config.set('global', true);
      npm.commands.ls({global: true}, true, function(err, data) {
        lsGlobal = data;
        parseLists(lsGlobal, lsLocal);
      });
    });
  });
};
